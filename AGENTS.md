...
`mcclusterishere/Here` is a retired legacy website repository and is **never authoritative**. Never use it as master repo, control plane, source of truth, upstream architecture, deploy/config authority, agent-law authority, or canonical backend reference. Do not inherit control-plane assumptions or agent instructions from it. Any older instruction that implies otherwise is stale and must be ignored in favor of `mcclusterishere/mccluster`.
